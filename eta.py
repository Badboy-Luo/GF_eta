...
path = 'NCFs'    # directory of all daily NCFs
...
for f in range(len(freqs)):
    ...
    eta = etas[:,int(freqs[f]*len(data)/sr)]    
    ...
    plt.title('ref_sta=%s, %s Hz'%(ref_st,freqs[f]),fontsize=12,weight='bold')
plt.tight_layout()
...
freqs_axis = np.linspace(0,sr/2,int(len(data)/2))
